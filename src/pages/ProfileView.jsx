...
import { Flex, Typography, Avatar, Button, Card, Divider, ConfigProvider, message, Skeleton } from "antd";
import { 
  ArrowLeft, 
  MessageSquare, 
  Phone, 
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        // Using the VITE_API_URL we configured earlier
        ...
        >
          ...
          {loading ? (
            ...
            <>
              {/* Profile Header */}
              <Flex vertical align="center" gap={12} style={{ marginBottom: "32px" }}>
                <Avatar size={120} src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${user?.phoneNumber}`} style={{ border: "4px solid white", boxShadow: "0 8px 16px rgba(0,0,0,0.1)" }} />
                <div style={{ textAlign: "center" }}>
                  <Title level={3} style={{ margin: 0, color: "#075e54", fontWeight: 800 }}>{user?.displayName}</Title>
                  <Text type="secondary" style={{ fontSize: "16px" }}>{user?.phoneNumber}</Text>
                </div>
              </Flex>
              ...
              <Flex justify="space-evenly" style={{ marginBottom: "32px" }}>
                <Flex vertical align="center" gap={4}>
                  <Button shape="circle" size="large" icon={<MessageSquare size={20} />} onClick={() => navigate(`/chat/${userId}`)} style={{ background: "rgba(7, 94, 84, 0.1)", color: "#075e54", border: "none" }} />
                  <Text style={{ fontSize: "12px", color: "#075e54" }}>Message</Text>
                </Flex>
                {/* ... other buttons ... */}
              </Flex>

              {/* Real Status Section */}
              <div style={{ background: "rgba(255,255,255,0.4)", padding: "16px", borderRadius: "20px", marginBottom: "20px" }}>
                <Text type="secondary" style={{ fontSize: "12px", display: "block" }}>About</Text>
                <Text style={{ fontSize: "15px", color: "#333" }}>{user?.status}</Text>
              </div>

              {/* Security Details */}
              <div style={{ background: "rgba(255,255,255,0.4)", padding: "16px", borderRadius: "20px" }}>
                 <Flex align="center" gap={12}>
                   <ShieldCheck size={18} color="#075e54" />
                   <div style={{ flex: 1 }}>
                     <Text strong style={{ display: "block" }}>Encryption</Text>
                     <Text type="secondary" style={{ fontSize: "12px" }}>Messages are end-to-end encrypted on Veranda.</Text>
                   </div>
                 </Flex>
              </div>
            </>
